...
class TutorMessagesController extends Controller
{
    public function index() {
        ...
        return view('tutor.messenger.index', compact('threads'));
    }
    ...
    public function show($id) {
        try {
            ...
            return redirect()->route('tutor.messages');
        }
        $users = User::whereNotIn('id', $thread->participantsUserIds(Auth::user()->id))->get();
        $thread->markAsRead(Auth::user()->id);
        return view('tutor.messenger.show', compact('thread', 'users'));
    }

    public function create($id) {
        if (is_numeric($id)) {
            $user = User::where('id', $id)->first();
            if (Auth::user()->profile->verified === null) {
                abort(403, 'You are not allowed to send message to this user');
            }
            if ($user->hasRole('tutor')) {
                abort(403, 'You are not allowed to send message to this user');
            }
            return view('tutor.messenger.create', compact('user'));
        } else {
            if ($id === 'support') {
                return view('tutor.messenger.adminsend');
            }
    ...
    public function update(Request $request, $id) {
        ...
        return redirect()->route('tutor.messages.show', $id);
    }
