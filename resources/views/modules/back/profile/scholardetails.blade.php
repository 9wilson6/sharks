    ...
    <div class="panel-body">
        <table class="table table-bordered table-hover table-striped">
            <tr>
                <td><b>Scholar</b></td>
                <td>{{ $userdetails->name }}
                    @if($level->acelevel($userdetails->id) == 'Top tutor')
                      <img src="{{ asset('img/level1.png') }}" width="80px" height="50px">
                    @endif
                    @if($level->acelevel($userdetails->id) == 'Verified tutor')
                      <img src="{{ asset('img/level2.png') }}" width="80px" height="50px">
                    @endif
                    @if($level->acelevel($userdetails->id) == 'Shark')
                      <img src="{{ asset('img/level3.png') }}" width="80px" height="50px">
                    @endif 
                    <h4>{{ $level->acelevel($userdetails->id) }}</h4></td>
            </tr>
            <tr>
