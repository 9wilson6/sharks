<template>
    ...
    <div v-if="orderdetails" class="panel-body">
     <div class="table-responsive">
        <table class="table table-striped table-hover">
            <thead>
                <tr>
                    <th>Posted By</th>
                    <th><a :href="'/admin/clients/student-profile/'+ orderdetails.student_id">{{ orderdetails.student.fullname }} ({{ orderdetails.student.name }})</a></th>
                </tr>
            </thead>
            <tbody>
                <tr>
        ...
        <a v-for="(attachment, index) in orderdetails.attachments" :key="index" :href="'/admin/orderuploads/get/' +attachment.order_id+ '/' +attachment.filename" class="btn btn-danger" v-else> {{ attachment.filename }} </a>
    </div>        
...
export default {
    computed: {
        ...
        orderdetails(){
            if (Object.entries(this.GetOrderDetails).length != 0) {
