...
function isModerator($userid,$groupid,$setModerator=false){
	global $moderatorUserIDs;
	...
	if(!isset($_SESSION['cometchat']['group']['group_'.$groupid]['user_'.$userid]['isModerator'])){
		if($setModerator==0){
			if(in_array($userid, $moderatorUserIDs)){
				$setModerator = true;
			}else{
				...
				$query = sql_query($sql,array(),1);
				if($room = sql_fetch_assoc($query)){
					$setModerator = $room['createdby'] == $userid;
				}
			}
		}
		$_SESSION['cometchat']['group']['group_'.$groupid]['user_'.$userid]['isModerator'] = $setModerator;
	}
	return $_SESSION['cometchat']['group']['group_'.$groupid]['user_'.$userid]['isModerator'];
}
function checkpassword() {
	...
	$response = array();
	$chatroomname = '';
	$groupid = sql_real_escape_string($_REQUEST['id']);
	$joinedChatroomIds = array();
	...
	if((!empty($_REQUEST['type']) && $_REQUEST['type']==1) && empty($_REQUEST['silent']) && !in_array($groupid, $joinedChatroomIds)) {
		...
		if($room = sql_fetch_assoc($query)){
			if (!empty($room['password']) && (empty($_REQUEST['password']) || ($room['password'] != $_REQUEST['password']))) {
				...
				if ($room['createdby'] == $userid ) {
					isModerator($userid,$groupid,true);
					$owner = "1";
				}
				$key = '';
				if( defined('KEY_A') && defined('KEY_B') && defined('KEY_C') ){
				...
				$response=array('s' => 'JOINED',
					...
					'userid' => $userid,
					'ismoderator' => "".ismoderator($userid,$groupid),
					'push_channel' => 'C_'.md5($channelprefix."CHATROOM_".$groupid.BASE_URL).getPlatformSuffix($pushplatformsuffix)
					);
				$responseLeg = md5('chatroom_'.$groupid.$key)."^".$owner."^".$userid."^".ismoderator($userid,$groupid);
				//Store chatroom name in session for push notifications
				$_SESSION['cometchat']['chatrooms']=array('_'.$groupid=>array('id'=>$groupid,'name'=>$room['name']));
				$_SESSION['cometchat']['chatroom']['n'] = base64_encode($room['name']);
				$_SESSION['cometchat']['chatroom']['id'] = $groupid;

				if(!in_array($groupid, $joinedChatroomIds)) {
					array_push($joinedChatroomIds,$groupid);
					$_SESSION['cometchat']['cometchat_joinedchatroomids'] = $joinedChatroomIds;
				}
			}
		}
...
function inviteusers() {
	...
	global $close;
	global $embedcss;
	$groupid = sql_real_escape_string($_REQUEST['roomid']);
	$inviteid = sql_real_escape_string($_REQUEST['inviteid']);
	$roomname = sql_real_escape_string($_REQUEST['roomname']);
	$base_url = BASE_URL;
	$response = array();
	$invitedusers = array();
	...
	if(!empty($_REQUEST['invite'])){
		...
		foreach ($_REQUEST['invite'] as $user) {
			$decodedroomname = base64_decode(rawurldecode($roomname));
			$invitemessage = $chatrooms_language[18].' '.$decodedroomname.' <a href="javascript:jqcc.cometchat.joinChatroom(\''.$groupid.'\',\''.$inviteid.'\',\''.$roomname.'\')">'.$chatrooms_language[19].'</a>';
			$invitedusers[] = $user;
			$sentinfo = sendMessage($user,$invitemessage,1);
			addUsersToChatroom($groupid, $user);
			$processedMessage = $_SESSION['cometchat']['user']['n'].": ".$chatrooms_language['chatroom_invite']." ".$decodedroomname;
			if($sentinfo != ''){
...
function kickUser() {
	...
	if (empty($_REQUEST['kick']) && !ismoderator($userid,$id)) {
		echo 0;
		exit;
	}
...
function banUser() {
	...
	if (empty($_REQUEST['ban']) && !ismoderator($userid,$id)) {
		echo 0;
		exit;
	}
...
function unbanusers() {
	...
	global $chatrooms_language, $close, $embedcss, $userid;
	$base_url = BASE_URL;
	$groupid = sql_real_escape_string($_REQUEST['roomid']);

	if(!ismoderator($userid,$groupid) || empty($_REQUEST['unban'])){
		echo '0';
		exit;
	}
	$inviteid = '';
	if(!empty($_REQUEST['inviteid'])){
		$inviteid = sql_real_escape_string($_REQUEST['inviteid']);
	}

	$roomname = sql_real_escape_string(rawurldecode($_REQUEST['roomname']));
	$decodedroomname = base64_decode($roomname);

	$unban = $_REQUEST['unban'];
	if(!is_array($unban)){
		$unban = explode(',',ltrim(rtrim($_REQUEST['unban'],']'),'['));
	}
	foreach ($unban as $user) {
		if(!empty($user)){
			...
			addUsersToChatroom($groupid, $user);
			sendMessage($user,$chatrooms_language[18].$decodedroomname. " <a href=\"javascript:jqcc.cometchat.joinChatroom('".$groupid."','".$inviteid."','".rawurlencode($roomname)."')\">".$chatrooms_language[19]."</a>",1);
		}
	}
