...
if(!empty($_REQUEST['admin'])){
	...
	if(file_exists($jsfile)&&DEV_MODE!=1){
		...
		if(($type!='core'||$name!='default')&&($type!='extension'||($type=='extension'&&$name=='jabber'))&&($type!='external')){
			...
			$cometchat['settings']['dockedAlignToLeft'] = $dockedAlignToLeft; //Set to yes, if docked layout has to be aligned to left.
			$cometchat['settings']['dockedChatListAudioCall'] = $dockedChatListAudioCall; //Set to yes, if docked layout has to be aligned to left.
