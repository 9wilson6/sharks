...
function addEmbedIframe(iframeObj,callbackfn) {
	if(iframeObj.module=="synergy" || iframeObj.module=="chatrooms"){
		...
		if(jqcc('link[href*="'+dynamicCDNUrl+'cometchatcss.php"]').length<=0 && cbfn!='desktop') {
			...
			addScript('<?php echo getDynamicScriptAndLinkTags(array('layout'=>'embedded', 'ext'=>'js'));?>');
		}else{
			jqcc('link[href*="'+dynamicCDNUrl+'cometchatcss.php"]').attr('href','<?php echo getDynamicScriptAndLinkTags(array('layout'=>$cc_layout,'urlonly'=>1, 'ext'=>'css'));?>');
		}
	}
